...
class myContext extends sfContext {
    ...
    public function isDeveloper() {
        return $this->getUser()->isAuthenticated() && $this->getUser()->getGuardUser()->username == 'admin';
    ...
    {
        return isset($_SESSION['lang']) ? $_SESSION['lang'] : MyConfig::get('project_lang');
    }
    ...
    public function checkDemoConnection() {
        //dummy
        return;
        ...
        if ($this->isDemo()) {
            ...
            $c = $manager->getConnection('multilang');
            $dsn = $c->getOption('dsn');
            if(!preg_match("/_$lang\$/", $dsn))
                $dsn .= '_' . $lang;
            $c->setOption('dsn', $dsn);
            /*
            echo $c->getDatabaseName();
            $c2 = $manager->getConnection('doctrine');
            echo $c2->getDatabaseName();
            die;
             * 
             */
        }
